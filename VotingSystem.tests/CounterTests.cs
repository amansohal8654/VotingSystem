...
namespace VotingSystem.tests
{
    ...
    public class CounterManager
    {
        public void ResolveExcess(List<Counter> counters)
        {
            ...
            var highestCounter = counters.Where(x => x.Percent == highestPercent).ToList();
            
            if(highestCounter.Count == 1)
            {
                highestCounter.First().Percent += excess;
            }
            else if(highestCounter.Count < counters.Count)
            {
